...
run_self_variance_once() {
  ...
  local raw start end status version commit digest result observed_cost derived_credits
  ...
  observed_cost=$(python3 - "$raw" <<'PY'
import json,sys
cost=None
for line in open(sys.argv[1],encoding="utf-8",errors="replace"):
 try: event=json.loads(line)
 except json.JSONDecodeError: continue
 if event.get("type")=="step_finish": cost=event.get("part",{}).get("cost")
print("null" if cost is None else cost)
PY
)
  if [[ "$observed_cost" == null ]]; then derived_credits=null; else derived_credits=$(derive_copilot_credits github-copilot USD copilot_provider_reported "$observed_cost"); fi
  set +e
  FIXTURE="$fixture" RAW="$raw" OUTPUT="$output" STATUS="$status" START="$start" END="$end" VERSION="$version" COMMIT="$commit" DIGEST="$digest" DERIVED_CREDITS="$derived_credits" python3 <<'PY'
...
comparable=lambda r: r.get("classification")=="VALID" and r.get("provider")=="github-copilot" and r.get("pricing_regime")=="standard" and r.get("credit_report",{}).get("cost_unit")=="USD" and r.get("credit_report",{}).get("cost_source")=="copilot_provider_reported" and isinstance(r.get("credit_report",{}).get("derived_credits"),(int,float))
print(sum(r.get("classification")=="VALID" for r in rows),len(rows),sum(r["credit_report"]["derived_credits"] for r in rows if comparable(r)))
PY
)
  (( valid < 3 )) || return 1
